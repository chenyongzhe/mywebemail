...
import cn.chen.dao.ReceiveMailMapper;
import cn.chen.pojo.ReceiveMail;
import cn.chen.service.ReceiveMailService;
...
public class ReceiveMailServiceImp implements ReceiveMailService
{
    ...
    public int addMail()  throws Exception
    {
        ...
        for(int i = 0; i < mails.size(); i++)
        {
            ...
            System.out.println("\n插入数据+"+i);
            System.out.println(receiveMail.getFilenamelist());
            ...
            if(state == 1)
            {
                ...
                receiveMail.setUrl("http://127.0.0.1:8080/myEmail/receive/" + id);
...
//            System.out.println("邮件大小：" + msg.getSize() * 1024 + "kb");
            ...
            if (isContainerAttachment) {
                System.out.println("包含附件 正在准备下载");
                //String psth1=
               // mail.setFilenamelist();
                saveAttachment(msg, "C:\\apache-tomcat-7.0.92\\webapps\\myEmail\\static\\",mail); //保存附件
...
//            System.out.println("邮件正文：" + (content.length() > 100 ? content.substring(0,100) + "..." : content));
     ...
     * @throws IOException
     */
    public static void saveAttachment(Part part, String destDir,ReceiveMail mail) throws UnsupportedEncodingException, MessagingException,
            FileNotFoundException, IOException
    {
        if (part.isMimeType("multipart/*"))
        {
            ...
            for (int i = 0; i < partCount; i++)
            {
                ...
                if (disp != null && (disp.equalsIgnoreCase(Part.ATTACHMENT) || disp.equalsIgnoreCase(Part.INLINE)))
                {
                    ...
                    mail.setFilenamelist(decodeText(bodyPart.getFileName()));
                    saveFile(is, destDir, decodeText(bodyPart.getFileName()));
                }
                else if (bodyPart.isMimeType("multipart/*"))
                {

                    saveAttachment(bodyPart,destDir,mail);
                }
                else
                {
                    String contentType = bodyPart.getContentType();
        ...
        else if (part.isMimeType("message/rfc822"))
        {
            ...
            saveAttachment((Part) part.getContent(),destDir,mail);
        }
    }
